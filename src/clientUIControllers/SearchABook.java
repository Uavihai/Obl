...
public class SearchABook implements Initializable {
	...
	@FXML
	private TableView<Book> booksList;
	...
	public void chooseABookBtn(ActionEvent event) throws Exception {
		if (this.booksList.getSelectionModel().getSelectedItem() == null)
			 ...
			 * selectedBook.getNumberOfCopies() + " copies avaiable.");
			 * sourceFrame.setSelectedBook(selectedBook);
			 * sourceFrame.setLendAndReturnDates(selectedBook.isPopular());
			 * 
			 * if (selectedBook.getNumberOfCopies() == 0)
			 * sourceFrame.getLendBtn().setDisable(true); else if
			 * (!sourceFrame.getLendBtn().isDisable())
			 * sourceFrame.getLendBtn().setDisable(false);
			 */
			selectedBook = booksList.getSelectionModel().getSelectedItem();
			if (sourceFrameLend != null) {
				sourceFrameLend.setSelectedBook(this.selectedBook);
				sourceFrameLend.getSubscriberRow().setText(
						selectedBook.getTitle() + " | " + selectedBook.getNumberOfCopies() + " copies avaiable.");

				if (selectedBook.getNumberOfCopies() == 0)
					sourceFrameLend.getLendBtn().setDisable(true);
				else if (!sourceFrameLend.getLendBtn().isDisable())
					sourceFrameLend.getLendBtn().setDisable(false);
				sourceFrameLend = null;
			} else 
				if (sourceFrameReturn != null) {
				sourceFrameReturn.setBookToReturn(this.selectedBook);
				sourceFrameReturn.getSubscriberRow().setText(
						selectedBook.getTitle() + " | " + selectedBook.getNumberOfCopies() + " copies avaiable.");
	...
	private void search() {
		...
		Communicate.getInstance().sendRequestToServer(bookSearchRequest);
	...
	public void updateUIPostBookSearch(ArrayList<Book> matchBooks) {
		try {
			...
			Platform.runLater(() -> {
				 ...
				 * 
				 * if (empty || item == null || item.getTitle() == null) { setText(null); } else
	 ...
	 */
	public TableView<Book> getBooksList() {
		return booksList;
	}
	 ...
	 */
	public void setBooksList(TableView<Book> booksList) {
	...
	/**
	 * @param titleColumn the titleColumn to set
	 */
	...
	/**
	 * @return the copiesColumn
	 */
	public TableColumn<Book, String> getCopiesColumn() {
		return copiesColumn;
	}

	/**
	 * @param copiesColumn the copiesColumn to set
	 */
	public void setCopiesColumn(TableColumn<Book, String> copiesColumn) {
		this.copiesColumn = copiesColumn;
	}
